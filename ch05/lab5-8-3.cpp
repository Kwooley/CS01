...
int	main()
{
	...
	do{
		cout << " Enter two integers (begin, end such that begin < end, begin>0, end < 1300)\n";
		cin >> begin >> end;
	} while ( begin > end || begin < 0 || end > 1300 ); 

	cout << "The sequence of 2 to power N in the range from " << begin << " to " << end << endl;
	while (flag){
		pow *= 2;
		if ( pow < begin )
			continue;
